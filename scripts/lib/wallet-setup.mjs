// The wallet, and everything that must be true before a transaction.
//
// Extracted from deploy-contract.mjs when a second script needed the same
// setup. Copying it would have been faster and wrong: the wait below encodes
// the fix for `Custom error: 170` — a dust spend proof built against a
// half-synced wallet is rejected by the node — and a copy would have kept
// only one of the two callers correct.
//
// Nothing here submits anything. Callers confirm for themselves, because what
// each is about to write to the chain differs.
...
export const ASSETS = "contracts/build/eduproof";

// How long to wait for the dust wallet to catch up before giving up.
//
// Measured on Preprod on 2026-08-29: ~230 indices/second against ~1.46M
// indices, so a cold sync takes about two hours. The sync does not persist
// between processes, so a timeout costs the whole wait — better to be
// generous. Override with DUST_SYNC_TIMEOUT_MS for a quicker failure.
export const DUST_SYNC_TIMEOUT_MS = Number(
  process.env.DUST_SYNC_TIMEOUT_MS ?? 4 * 60 * 60 * 1000,
);
...
export async function toHexSeed(raw) {
...
/**
 * Everything a transaction needs: a checked environment, an open wallet,
 * and a dust balance that is actually spendable.
 *
 * The caller is responsible for closing the wallet.
 */
export async function openFundedWallet() {
  loadEnvLocal();
